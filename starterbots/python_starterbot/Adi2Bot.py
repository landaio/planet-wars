...
def DoTurn(pw):
    ...
    target = len(sorted_num_ships) - 1 if rand == 0 else 0
    source_planet = sorted_num_ships[target]
    ...
    # num ships
    if num_ships > 500:
        num_ships = source_planet.NumShips() / 2
    else:
        num_ships = source_planet.NumShips() / 1.2
